...
template <typename T>
static unsigned int	how_many_packets(T &array, unsigned int packetsize)
{
	return array.size() / packetsize;
}

static unsigned int	actual_index(unsigned int packetsize, unsigned int packetindex)
{
	return (packetindex - 1) * packetsize;
}

template <typename T>
static int	packet_value(T &array, unsigned int packetsize, unsigned int packetindex)
{
	return array.at(actual_index(packetsize, packetindex) + packetsize - 1);
}

template <typename T>
static void	print_vector(T &array)
{
...
template <typename T>
static bool	is_sorted_vector(T &array)
{
...
template <typename T>
static void	insert_packet_at_index(T &array, unsigned int packetsize, unsigned int destpacketindex, T &loser, unsigned int loserpacketindex)
{
...
template <typename T>
static T	jacob_sequence(T &array, unsigned int packetsize)
{
	T	jacob;
	...
	while (newJacob <= max_size)
	{
		...
		newJacob = *(jacob.end() - 1) + 2 * *(jacob.end() - 2); // https://en.wikipedia.org/wiki/Jacobsthal_number
	}
	jacob.push_back(newJacob);
	return jacob;
}
 ...
 */
template <typename T>
static void	loser_packet(T &array, unsigned int packetsize, unsigned int packetindex, T &destination)
{
...
template <typename T>
static void separate_losers_from_winners(T &array, T &loser, unsigned int packetsize)
{
	int i = 1;
...
static int max_nb_comparisons(int nb)
{
	...
	for (int i = 1; i <= nb; ++i)
	{
		double log2 = std::log(0.75 * i) / std::log(2); // p185 property no 14
